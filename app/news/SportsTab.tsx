...
let   cachedSports: { ts: number; data: Article[] } | null = null;
...
export default function SportsTab() {
  ...
  useEffect(() => {
    ...
    (async () => {
      /* use cache if fresh */
      if (cachedSports && Date.now() - cachedSports.ts < CACHE_TTL) {
        setArticles(cachedSports.data);
        return;
      }
      ...
      try {
        ...
        if (!cancel) {
          cachedSports = { ts: Date.now(), data: sportsNews };
          setArticles(sportsNews);
        }
      } catch (e: any) {
        console.error(e);
