...
// Import the Mixpanel tracking functions.
import { trackEvent } from "@/utils/mixpanel";

// ======================================================================
// PATCH dat.gui's destroy() method so that it does NOT remove its DOM element.
// This avoids errors when the element is not a child of its parent.
// ======================================================================
if (typeof window !== "undefined" && GUI && GUI.prototype) {
...
const CymaticPage: React.FC = () => {
  // Ref for the container that will hold the Three.js canvas.
  const containerRef = useRef<HTMLDivElement>(null);
  // (Optional) Ref for simulation API functions (if needed elsewhere).
  const simulationRef = useRef<{
    updateFrequency: (newFreq: number) => void;
    startSound: (newFreq: number) => void;
    stopSound: () => void;
  } | null>(null);
  ...
  // On mount, track that the page was viewed.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // ----------------------------------------------------------------
    // Simulation Parameters – including new options:
    // - numSand: number of sand particles.
    // - plateRadius: for circle board.
    // - plateShape: "circle" or "square".
    // - plateSize: for square board (full side length).
    // - frequency: Hz used for simulation and for sound.
    // - boardColor & particleColor: initial colors.
    // - boardCycle & particleCycle: whether to cycle colors.
    // - colorSpeed: how fast the hue cycles.
    // ----------------------------------------------------------------
    ...
    // Additional 3D effect: a point light whose color cycles.
    ...
    const initBoard = () => {
      ...
      if (simulationParams.plateShape === "circle") {
        ...
        // Convert to BufferGeometry via vertices for vertex manipulation.
    ...
    const initScene = () => {
      ...
      // The white boundary circle has been removed.
      initParticles();
    ...
    // Add buttons to play and stop sound using the current frequency.
    gui.add({ playSound: () => { startSound(simulationParams.frequency); } }, "playSound")
      .name("Play Sound");
    gui.add({ stopSound: () => { stopSound(); } }, "stopSound")
      .name("Stop Sound");

    // Reparent the dat.gui DOM element into our custom container.
